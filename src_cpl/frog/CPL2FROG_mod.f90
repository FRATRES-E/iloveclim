...
!-----|--1----+----2----+----3----+----4----+----5----+----6----+----7----+----8----+----9----+----0----+----1----+----2----+----3-|
      ...
      module CPL2FROG_mod
       ...
       real(kind=sp), dimension(:,:,:), allocatable :: temp2vamper, dsnow2vamper
       integer :: storing_time_step
...
!~        real, intent(out)   :: outParam
        integer, intent(in) :: nb_days_coupling
...
!>    @bug Description of the stupid sticky bug that we know exist there but is not corrected yet!
        ...
        ALLOCATE(temp2vamper(nlat,nlon,nb_days_coupling))
        ALLOCATE(dsnow2vamper(nlat,nlon,nb_days_coupling))
        storing_time_step = 0
        returnValue = .true.
...
!>    @bug Description of the stupid sticky bug that we know exist there but is not corrected yet!
        ...
        storing_time_step = storing_time_step + 1

        temp2vamper(:,:,storing_time_step) = tsurf1(:,:)
        dsnow2vamper(:,:,storing_time_step) = dsnow(:,:,iwater)
