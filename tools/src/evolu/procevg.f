...
c-----------------------------------------------------------------------
      ...
      write(6,*) 'Nom du fichier a traiter (max 40 cc) ?'
      read(5,'(A)') fichr
      write(6,*) 'Nom du fichier resultat  (max 40 cc) ?'
      read(5,'(A)') fichw
...
c--Impression a l'ecran (pour faciliter le choix) :
     ...
     &      jm,' (= jm) enregistrements'
      ...
      write(6,'(A)') ' Input Nb de colonne a Ajouter (+Nb) '
     &             //'/ a Suprimer (-Nb) ? (0 => autre)'
      read(5,*) modcol
...
c- Condense le fichier par Moy/Min/Max, par groupe de "inter1/inter0" lignes
     ...
     &   ' Applique Moyenne ou Min/Max a chaque groupe de lignes.'
        write(6,'(2A,I5,A)')' Nb_d''iter par groupe de lignes ?',
     &   ' (Nb_actuel=', inter0, ') ; Input inter1= ?'
        read(5,*) inter1
        write(6,'(2A)') ' Nb_Variables a traiter par Min/Max ?',
     &   ' (defaut=Moy.) ; Input nbmnx = ?'
        read(5,*) nbmnx
