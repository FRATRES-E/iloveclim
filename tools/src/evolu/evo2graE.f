...
C     write(6,*) len(tit1), len(titvar(1))
      write(6,*) 'Name of the data file ?'
      call rdstdin(numf,fildat)
...
c entree au clavier du numero du tableau (ou bloc) a traiter
 ...
 120    continue
        write(6,'(A)') 'Number of the arrays to vizualise ? '
     &     // '(-num < 0 for prolongation ; num > 0 if last)'
...
C      jpxy=1*4
      ...
      open (8, FILE='gra'//fildat//'.dat', FORM='UNFORMATTED',
...
C       write(8,REC=IREC) (sort(i,j),J=1,jm)
      ...
      close(8)
      write(6,*) 'write Z OK'
      open (9, FILE='gra'//fildat//'.ctl')
      write (9,'(A6,A12)') 'dset ^','gra'//fildat//'.dat'
