...
!
      ...
      real(kind=dblp), dimension(imax,jmax) :: ut, sm, s0, sx, sxx
     >               , sy, syy, sxy, f0, fx, fxx, fy, fyy, fxy, fm      


      real(kind=dblp)::  alg, alg1, alg1q, bet, dt, crh, slpmax, s1max
     >               , s1new, s2new, zin0, alf, alfq, alf1, alf1q, temp
     
      integer(kind=ip):: i, j, ii
      
      
c~       dimension ut(imax,jmax)
...
!
!---5----|----5----|----5----|----5----|----5----|----5----|----5----|72--5----|
!  4) Calculate fluxes and moments between boxes i<-->i+1              |
!-----------------------------------------------------------------------
!
      do 80 j=js1,js2
        do 70 i=is1(j)-1,is2(j)
!
!--4.1. FLUX FROM i TO i+1 WHEN u.GT.0.0. 
!----------------------------------------
!     
          ...
          fyy(i,j) = alf*syy(i,j)
!
!  4.1.1. Readjust moments remaining in the box.
!
...
!
!--4.2. FLUX FROM i+1 TO i WHEN u.LT.0.0.
!----------------------------------------
!
          ii         = i+1
...
!
!  4.2.1. Readjust moments remaining in the box. 
!
          ii       = i-1
...
!
!--5.1. FLUX FROM i to i+1 IF u.GT.0.0.
!--------------------------------------
!
          ii       = i-1
...
!
!--5.2. FLUX FROM i+1 to i IF u.LT.0.0.
!--------------------------------------
!
