...
!      MODULE: [defgrid_mod]
!
!>     @brief Orchestrateur d'initialisation de la grille oceanique CLIO.
!!            Delegue chaque groupe de calcul a une sous-routine privee dediee.
!
!      DESCRIPTION:
!>     La routine publique defgrid(nflag) met a zero les champs globaux, calcule les constantes de resolution, puis enchaine
!!     neuf etapes de calcul (G1..G5c). Les coordonnees et metriques (G1/G2) sont les SEULS blocs dependant du type de grille ;
!!     toutes les autres procedures (G3a..G5c) sont agnostiques a la grille et communiquent par arguments explicites.
!!
!!     Sequence d'appel (ordre de dependance) :
!!       [init]  mise a zero des champs + constantes de resolution / indices geographiques
!!       [G1]    compute_grid_coords        -- coordonnees lon/lat et metriques locales       *** DEPEND DE LA GRILLE ***
!!       [G2]    compute_metric_coeffs      -- cmx/cmy/smx/smy/cmxy/smxy/fs2cor/covrai          *** DEPEND DE LA GRILLE ***
...
!!       [sect8] write_mouchard_defgrid     -- sortie de controle (nflag==2 seulement)
!!
!>     POUR CHANGER DE TYPE DE GRILLE : remplacer UNIQUEMENT compute_grid_coords (G1) et compute_metric_coeffs (G2).
!!
!!     Tableaux locaux intermediaires propres a l'initialisation (passes entre G1/G2/G3a, non stockes dans les modules) :
!!       dx1_loc, dx2_loc   : pas angulaires adimensionnes en x/y (rad)      -- sortie G1
!!       h1_loc,  h2_loc    : coefficients metriques x/y au pt S (m)         -- sortie G1
!!       d1d2_loc, d2d1_loc : d(h1)/dy, d(h2)/dx au pt S (m/rad)             -- sortie G1
!!       unszw_loc          : 1/zw(k)                                        -- sortie G3b, entree G4a
!!       zbath_loc          : profondeur brute des centres (bath.om)         -- sortie G3b
!!       dzbath_loc         : epaisseur brute des niveaux (bath.om)          -- sortie G3b
!-----|--1----+----2----+----3----+----4----+----5----+----6----+----7----+----8----+----9----+----0----+----1----+----2----+----3-|

      module defgrid_mod

       use global_constants_mod, only: dblp => dp, ip
       use defgrid_compute_mod,  only: compute_grid_coords              &
                                     , compute_metric_coeffs
       use defgrid_kernel_mod,   only: compute_ice_metrics              &
                                     , read_bathymetry_and_vscale       &
                                     , build_ocean_mask                 &
                                     , build_domain_indices             &
                                     , compute_integrals                &
                                     , compute_surface_physics          &
                                     , define_ice_shelves
...
!
!      DESCRIPTION:
!>     nflag = 1 : initialisation standard (premier appel)
!!     nflag = 2 : idem + ecriture de controle dans le fichier "mouchard"
!!     nflag = 3 : mise a jour de la bathymetrie (paleo-simulations)
!-----|--1----+----2----+----3----+----4----+----5----+----6----+----7----+----8----+----9----+----0----+----1----+----2----+----3-|
...
!------------------------------------------------------------------------------
!      Variables locales propres a l'orchestrateur
!------------------------------------------------------------------------------
       integer(ip) :: kmaxp1, i, j, k, ns, nn, npt0v
       integer(ip) :: ipt0v(10), jpt0v(10)

!      Tableaux intermediaires G1 -> G2 -> G3a
!      (non stockes dans les modules car propres a l'initialisation)
...
!      Tableaux intermediaires G3b -> G4a
       real(dblp), dimension(kmax)   :: zbath_loc, dzbath_loc, unszw_loc

!      Tableau de separation grille AA, specifique a la resolution CL15/CL30
...
!      Sauvegarde des masques (paleo-simulations avec bathymetrie variable)
#if ( BATHY >= 2 )
...
!      Variable locale pour le coefficient de Bering avant calcul geostrophique
       real(dblp)  :: bering_init

       kmaxp1 = kmax + 1

!==============================================================================
!      ETAPE 0 : INITIALISATION DES TABLEAUX GLOBAUX
!                Mise a zero de tous les champs avant calcul.
!==============================================================================

!-- Bornes i : initialisees a "vide" (is1=imax, is2=1)
       do j=1,jmax
         is1(j)=imax  ; is2(j)=1
         iu1(j)=imax  ; iu2(j)=1
...
!-- Initialisation conditionnelle (pas lors des updates paleo)
       if (nflag.ne.3) then
...
!-- Traceurs scalaires : initialisation au profil de reference scal0
         do ns=1,nsmax
           do k=1,kmax
             nrap(k,ns)=0
             do j=1,jmax
...
!-- Terme PME (Precipitation - Evaporation) pour les scalaires de surface
         do ns=1,nsmax
           do j=1,jmax
             do i=1,imax
               scs(i,j,ns)=scpme(ns)
       ...
       else  ! nflag == 3 : update bathymetrie paleo
#if ( BATHY >= 2 )
         tms_old(:,:,:)=tms(:,:,:)
         tmu_old(:,:,:)=tmu(:,:,:)
         tms(:,:,:)=0.
...
!==============================================================================
!      ETAPE 1 : CONSTANTES DE RESOLUTION ET INDICES GEOGRAPHIQUES
!                Calcul des bornes actives, resolutions dx/dy et indices de detroits.
!==============================================================================
       ims1=2 ; ims2=imax-1
       js1=2  ; js2=jmax-1
       ks1=1  ; ks2=kmax
...
!-- Indices geographiques cles (jeq, dateline, Bering) via geogra
       call geogra(js1, js2, jeq, jdl1, jdl2, ijsdl, ijudl, iberp, ibera)

!-- Bornes de la correspondance cyclique E<->W
       if (ltest.lt.1) then
         jcl1=js2 ; jcl2=js1   ! bassin ferme : boucle vide (jcl1>jcl2)
       else
         jcl1=js1 ; jcl2=jeq   ! grille monde : correspondance jusqu'a l'equateur
       endif

!-- Taille de maille en metres
       if (ltest.lt.0) then
         dx=1000. ; dy=1000.   ! grille cartesienne (cas test)
       else
         dx=dlong*radian*rterre ; dy=dlat*radian*rterre
       endif
       unsdx=1./dx ; unsdy=1./dy ; uns2dx=.5/dx ; uns2dy=.5/dy

!-- Indices Bering decales de -1 (pour les operateurs aux demi-pas)
       jberpm=jberp-1 ; iberpm=iberp-1
       jberam=jbera-1 ; iberam=ibera-1

!-- Iles "sans surface de vitesse" (pts scalaires isoles, ex: Spitzberg)
       ipt0v(1)=109 ; jpt0v(1)=55 ; npt0v=0  ! desactive (npt0v=0)

!-- Points avec vitesse oceanique mais sans vitesse glace (Mer de Barents)
       npo1i0=2
       ipo1i0(1)=102 ; jpo1i0(1)=56
       ipo1i0(2)=111 ; jpo1i0(2)=56

!-- Sauvegarde du coefficient Bering avant recalcul geostrophique en G4b
       bering_init=bering

!==============================================================================
!      [G1] COORDONNEES GEOGRAPHIQUES ET METRIQUES LOCALES
!           *** SEUL BLOC A REMPLACER POUR UN AUTRE TYPE DE GRILLE ***
!==============================================================================
...
!==============================================================================
!      [G2] COEFFICIENTS METRIQUES
!           *** A REMPLACER CONJOINTEMENT AVEC G1 POUR UN AUTRE TYPE DE GRILLE ***
!==============================================================================
...
!==============================================================================
!      [G3a] METRIQUES POUR LA DYNAMIQUE DE LA GLACE DE MER
!            Agnostique a la grille.
!==============================================================================
...
!==============================================================================
!      [G3b] LECTURE DE LA BATHYMETRIE ET CALCUL DE L'ECHELLE VERTICALE
!            Agnostique a la grille.
!==============================================================================
...
!==============================================================================
!      [G4b] CALCUL DES INDICES DE DOMAINE ET DU COEFFICIENT DE BERING
!            Agnostique a la grille.
!==============================================================================
...
!==============================================================================
!      [G5a] GRANDEURS INTEGRALES : SURFACES, VOLUMES, COINS, PENTES
!            Agnostique a la grille.
!==============================================================================
...
!==============================================================================
!      ETAPE 8 : ECRITURE DE CONTROLE DANS LE FICHIER "MOUCHARD" (si nflag==2)
!                Placee APRES G5c pour que toutes les variables soient calculees.
!==============================================================================
       if (nflag.eq.2) then
...
!
!>     @brief Ecriture de controle dans le fichier "mouchard" (debogage, nflag==2).
!
!      DESCRIPTION:
!>     Extrait de la section 8 originale de defgrid. Appelee uniquement lorsque nflag==2, avec en arguments les sorties des
!!     neuf sous-routines G1..G5c, afin de tracer sommes de controle, profils verticaux, indices de domaine et carte ASCII
!!     du fond. N'a aucun effet de bord sur l'etat du modele : sortie diagnostique seulement.
!-----|--1----+----2----+----3----+----4----+----5----+----6----+----7----+----8----+----9----+----0----+----1----+----2----+----3-|
