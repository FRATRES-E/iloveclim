...
!  Resolution complete du mode barotrope, Sans Filtre sur "eta" (ahe = 0)
...
!dmr [NOEQUI]      dimension unsdet(imax,jmax)
!dmr [NOEQUI]      equivalence ( unsdet(1,1) , phihhh(1,1,3) )

!--pour raison de place memoire :
      real(kind=dblp), dimension(imax)     :: phixj, phixju, phixjv
...
!---+----1----+----2----+----3----+----4----+----5----+----6----+----7-|--+----|
!  1 ) calcul de l'elevation, cas sans Filtre (ahe nul) :              |
!-----------------------------------------------------------------------

!--Debut de la 1ere boucle externe sur l'indice de latitude j :
!$DIR SPP LOOP_PARALLEL
!$DIR SPP LOOP_PRIVATE(i,ccdif,phixj)
      do 200 j=js1,js2
!-----
!--Calcul des Flux :
...
!--Calcul des Flux :
 ...
 110  continue
!     do 120 j=js1,1+js2
!      do 120 i=isf1(j),isf2(j)
!       phiy(i,j) = cmx(i,j,2) * (vb(i,j)+vb(i+1,j))
!120  continue
