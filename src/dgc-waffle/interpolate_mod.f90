...
#define INT_MODEL 1
      ...
      USE global_constants_mod, only: dp, ip, eps_dp
      ...
      logical function interpolate_init(nx,ny,nlat,nlon,latEcb,lonEcb   &
                ,YLAT,XLONG,nw,nz,ex,int_coord_tab,weights, sum_weights, maskout)
...
!-----|--1---------2---------3---------4---------5---------6---------7---------8---------9---------0---------1---------2---------3-|
       ...
       INTEGER(ip),DIMENSION(nw-1,0:nz,nx,ny),INTENT(out) :: int_coord_tab
...
!-----|--1---------2---------3---------4---------5---------6---------7---------8---------9---------0---------1---------2---------3-|
       ...
       REAL(dp), DIMENSION(nlat,nlon), INTENT(in), OPTIONAL :: maskout
...
!-----|--1---------2---------3---------4---------5---------6---------7---------8---------9---------0---------1---------2---------3-|
       ...
       REAL(dp), DIMENSION(nx,ny,nw,0:nz) :: tab_dat
...
!       3.4: create interpolation data with distance ...
       ...
       int_coord_tab(1,0,ii,jj)=NINT(tab_dat(ii,jj,1,0))
       int_coord_tab(2,0,ii,jj)=NINT(tab_dat(ii,jj,2,0))
...
!$ USE OMP_LIB
      ...
      INTEGER(ip), DIMENSION(nw-1,0:nz,nx,ny),INTENT(in) :: int_coord_tab
...
!$OMP END PARALLEL
       ...
       DO jj = 1, ny
         DO ii = 1, nx
           ...
           IF (sum_weights(ii,jj).gt.eps_dp) then
...
!dmr    nz is the number of neighbouring cells you want to interpolate with
           ...
           ELSE
              ...
              i = int_coord_tab(1,0,ii,jj)
              j = int_coord_tab(2,0,ii,jj)
              pfGi(ii,jj) = sxsnowG(j,i)
